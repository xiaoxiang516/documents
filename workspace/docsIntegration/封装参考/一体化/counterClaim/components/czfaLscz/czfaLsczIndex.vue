...
<template>
  <div class="common-app" v-loading="pageControl.pageLoading">
    ...
    <CommonTable
      :schema="pageObjInj.schemas[pageObjInj.entityIds.indexOf('Czfa2Lscz')]"
      :tableSchema="lsczObj.tableSchema"
      :tableUiSchema="lsczObj.tableUiSchema"
      :tableData="pageObjInj.formData.to_Czfa2LsczSet"
      :rules="lsczObj.rules"
      rowKey="Guid"
...
import { pageControl, Fn, lsczObj } from './index'
...
const add = () => {
  pageObjInj.formData.to_Czfa2LsczSet?.push({}) // 增加一行
}
const del = (idx: number) => {
  pageObjInj.formData.to_Czfa2LsczSet?.splice(idx, 1) // 删除一行
}
