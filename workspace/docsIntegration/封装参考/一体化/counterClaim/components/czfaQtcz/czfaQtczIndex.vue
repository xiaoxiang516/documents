...
<template>
  <div class="common-app" v-loading="pageControl.pageLoading">
    ...
    <CommonTable
      :schema="pageObjInj.schemas[pageObjInj.entityIds.indexOf('Czfa2Qtcz')]"
      :tableSchema="qtczObj.tableSchema"
      :tableUiSchema="qtczObj.tableUiSchema"
      :tableData="pageObjInj.formData.to_Czfa2QtczSet"
      :rules="qtczObj.rules"
      rowKey="Guid"
...
import { pageControl, Fn, qtczObj } from './index'
...
const add = () => {
  pageObjInj.formData.to_Czfa2QtczSet?.push({}) // 增加一行
}
const del = (idx: number) => {
  pageObjInj.formData.to_Czfa2QtczSet?.splice(idx, 1) // 删除一行
}
