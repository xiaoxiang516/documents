...
<template>
  <div class="common-app" v-loading="pageControl.pageLoading">
    ...
    <CommonTable
      :schema="pageObjInj.schemas[pageObjInj.entityIds.indexOf('Czfa2Jxcz')]"
      :tableSchema="jxczObj.tableSchema"
      :tableUiSchema="jxczObj.tableUiSchema"
      :tableData="pageObjInj.formData.to_Czfa2JxczSet"
      :rules="jxczObj.rules"
      rowKey="Guid"
...
import { pageControl, Fn, jxczObj } from './index'
...
const add = () => {
  pageObjInj.formData.to_Czfa2JxczSet?.push({}) // 增加一行
}
const del = (idx: number) => {
  pageObjInj.formData.to_Czfa2JxczSet?.splice(idx, 1) // 删除一行
}
