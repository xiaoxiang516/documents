...
<template>
  <div class="common-app" v-loading="pageControl.pageLoading">
    ...
    <CommonTable
      :schema="pageObjInj.schemas[pageObjInj.entityIds.indexOf('Czfa2Rycz')]"
      :tableSchema="ryczObj.tableSchema"
      :tableUiSchema="ryczObj.tableUiSchema"
      :tableData="pageObjInj.formData.to_Czfa2RyczSet"
      :rules="ryczObj.rules"
      rowKey="Guid"
...
import { pageControl, Fn, ryczObj } from './index'
...
const add = () => {
  pageObjInj.formData.to_Czfa2RyczSet?.push({}) // 增加一行
}
const del = (idx: number) => {
  pageObjInj.formData.to_Czfa2RyczSet?.splice(idx, 1) // 删除一行
}
