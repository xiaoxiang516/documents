...
// 表格校验
const validateData = async () => {
  // mergeK2CfgInReactiveObj覆盖了itemInfoRules
  return new Promise((resolve, reject) => {
    const list = Object.keys(outputPlanObj.itemInfoRules)
    for (const item of pageObjInj.formData.SjjcXxjhSet) {
      for (const key of list) {
        if (item[key] === null || item[key] === '' || item[key] === undefined) {
          ElMessage({
            message: `【${outputPlanObj.itemInfoRules[key][0].message}】为必填项`,
            type: 'error'
          })
          resolve(false)
          return
...
```ts
 if (Operation !== OPERTAION_KEY.SAVE.key) {
    const validate = validateForm(sourceHead.formData, sourceHead.rules)
    if (!validate) return
    // 提交时附件校验
    const index = tabList?.findIndex(item => item.value === 'to_AttachmentCzhzSet')
    const disposalAttachment = refList.value[index]
    if (disposalAttachment.checkAttachmentInfo && !disposalAttachment.checkAttachmentInfo()) return
  }
```

```ts
handlerDateFormat(res, ['SjFsdat', 'Erdat'])
export function handlerDateFormat(data：Array, props: string[] = []) {
