...
<template>
  <div class="common-app" v-loading="pageControl.pageLoading">
    ...
    <CommonTable
      :schema="pageObjInj.schemas[pageObjInj.entityIds.indexOf('Czfa2Clcz')]"
      :tableSchema="clczObj.tableSchema"
      :tableUiSchema="clczObj.tableUiSchema"
      :tableData="pageObjInj.formData.to_Czfa2ClczSet"
      :rules="clczObj.rules"
      rowKey="Guid"
...
import { pageControl, Fn, clczObj } from './index'
...
const add = () => {
  pageObjInj.formData.to_Czfa2ClczSet?.push({}) // 增加一行
  console.log(pageObjInj)
}
const del = (idx: number) => {
  pageObjInj.formData.to_Czfa2ClczSet?.splice(idx, 1) // 删除一行
}
