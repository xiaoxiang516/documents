...
<template>
  <div class="common-app" v-loading="pageControl.pageLoading">
    ...
    <CommonTable
      :schema="pageObjInj.schemas[pageObjInj.entityIds.indexOf('Czfa2Czhz')]"
      :tableSchema="czhzObj.tableSchema"
      :tableUiSchema="czhzObj.tableUiSchema"
      :tableData="pageObjInj.formData.to_Czfa2CzhzSet"
      :rules="czhzObj.rules"
      rowKey="Guid"
...
import { pageControl, Fn, czhzObj } from './index'
...
const add = () => {
  pageObjInj.formData.to_Czfa2CzhzSet?.push({}) // 增加一行
}
const del = (idx: number) => {
  pageObjInj.formData.to_Czfa2CzhzSet?.splice(idx, 1) // 删除一行
}
